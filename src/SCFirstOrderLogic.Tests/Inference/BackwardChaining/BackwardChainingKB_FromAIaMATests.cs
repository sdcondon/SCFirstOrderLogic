...
public static class BackwardChainingKB_FromAIaMATests
{
    public static Test PositiveScenarios => TestThat
        .GivenTestContext()
        .AndEachOfAsync<BackwardChainingKB_FromAIaMA.Query>(async () =>
        [
            // trivial
    ...
    public static Test NegativeScenarios => TestThat
        .GivenEachOfAsync<BackwardChainingKB_FromAIaMA.Query>(async () =>
        [
            // no matching clause
            await MakeQueryAsync(
                query: IsEvil(X),
    ...
    private static async Task<BackwardChainingKB_FromAIaMA.Query> MakeQueryAsync(Sentence query, IEnumerable<Sentence> kb)
    {
