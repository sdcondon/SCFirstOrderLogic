...
public static class SubstitutionExtensions
{
    ...
    {
        // TODO-BUG?: is this conventional? otherwise the empty clause (which is conventially ⊥, a contradiction)
    ...
    /// <summary>
    /// <para>
    /// Ordinalises a formula; replacing all variable identifiers with the (zero-based) ordinal of where they
    /// first appear in a depth-first traversal of the formula.
    /// </para>
    /// <para>
    /// This is useful because it makes the original identifiers irrelevant but preserves distinctness, so that 
    /// e.g. P(X, X) is transformed to a formula that is identical to the transformation of P(Y, Y), but different
    /// to the transformation of P(X, Y). Making the original identifier irrelevant is useful when e.g. indexing.
    /// </para>
    /// </summary>
    /// <param name="formula">The formula to ordinalise.</param>
    /// <returns>The ordinalised formula.</returns>
    public static Formula Ordinalise(this Formula formula)
    {
        return formula.Accept(new VariableOrdinalisation());
    }
    ...
    /// <summary>
    /// <para>
    /// Formula transformation that converts all variable identifiers to the integer value of their order in a depth-first traversal of the formula.
    /// </para>
    /// <para>
    /// Useful in e.g. indexing, where we want to make the original identifier irrelevant when comparing formulas/clauses/terms.
    /// </para>
    /// </summary>
    private class VariableOrdinalisation : RecursiveFormulaTransformation
    {
