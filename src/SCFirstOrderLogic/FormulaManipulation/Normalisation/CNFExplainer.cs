...
{
    private readonly FormulaFormatter formulaFormatter;

    /// <summary>
    /// Initializes a new instance of the <see cref="CNFExplainer"/> class.
    /// </summary>
    /// <param name="formulaFormatter">The formatter to use when formatting formula elements as part of output.</param>
    public CNFExplainer(FormulaFormatter formulaFormatter)
    {
    ...
    public string ExplainNormalisationTerm(Term term)
    {
        ...
        {
            return $"some {formulaFormatter.Format(skolemFunctionIdentifier.VariableIdentifier)} from {formulaFormatter.Format(skolemFunctionIdentifier.OriginalFormula)}";
        }
        else if (term is VariableReference variable && variable.Identifier is StandardisedVariableIdentifier standardisedVariableIdentifier)
        {
            ...
            return $"a standardisation of {standardisedVariableIdentifier.OriginalIdentifier} from {formulaFormatter.Format(standardisedVariableIdentifier.OriginalFormula)}";
        }
        else
        {
