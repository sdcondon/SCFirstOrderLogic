...
/// <summary>
/// <para>
/// Base class for recursive asynchronous visitors of <see cref="Sentence"/> instances.
/// </para>
/// <para>
/// That is, a base class for asynchronous visitors in which the default implementation for any non-terminal
/// sentence/term element simply visits the element's children (in parallel) - and does nothing for terminal elements.
/// </para>
/// </summary>
public abstract class RecursiveAsyncSentenceVisitor : IAsyncSentenceVisitor, IAsyncTermVisitor
{
    /// <summary>
    /// Visits a <see cref="Sentence"/> instance.
    /// The default implementation just invokes the VisitAsync method appropriate to the type of the sentence (via <see cref="Sentence.Accept(ISentenceVisitor)"/>).
    /// </summary>
    /// <param name="sentence">The sentence to visit.</param>
    public virtual async Task VisitAsync(Sentence sentence)
    {
        await sentence.AcceptAsync(this);
    }
    ...
    public virtual async Task VisitAsync(ExistentialQuantification existentialQuantification)
    {
        await Task.WhenAll(
            ...
            VisitAsync(existentialQuantification.Sentence));
    }
    ...
    {
        await VisitAsync(negation.Sentence);
    }
    ...
    public virtual async Task VisitAsync(UniversalQuantification universalQuantification)
    {
        await Task.WhenAll(
            ...
            VisitAsync(universalQuantification.Sentence));
    }
