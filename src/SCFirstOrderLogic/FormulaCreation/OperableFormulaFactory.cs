...
/// <code>using static SCFirstOrderLogic.FormulaCreation.OperableFormulaFactory;</code>
/// This class provides a compromise between <see cref="FormulaFactory"/> and the full language integration provided by the types in the LanguageIntegration namespace.
/// The objects returned by this factory (are implicitly convertible to <see cref="Formula"/>s and) override operators in a way that is perhaps intuitive: | for disjunctions, &amp; for conjunctions, ! for negations, and == for the equality predicate.
/// For domain-specific formula elements (predicates &amp; functions), their operable surrogate is implicitly convertible from (and to) the regular type, so the recommendation is to create appropriate properties and methods to create them - declaring
...
/// </para>
/// <para>
/// <strong>N.B. #1:</strong> The real formula classes do not define these operators to keep them as lean and mean as possible.
/// </para>
/// <para>
/// <strong>N.B. #2:</strong> It's also probably worth noting that this approach's reliance on implicit conversion will result in some memory overhead compared to using FormulaFactory -
...
/// </para>
/// </summary>
public static class OperableFormulaFactory
{
    /// <summary>
    ...
    public static OperableFormula ForAll(OperableVariableDeclaration variableDeclaration, OperableFormula formula) =>
        new OperableUniversalQuantification(variableDeclaration, formula);
    ...
    public static OperableFormula ForAll(OperableVariableDeclaration variableDeclaration1, OperableVariableDeclaration variableDeclaration2, OperableFormula formula) =>
        new OperableUniversalQuantification(variableDeclaration1, new OperableUniversalQuantification(variableDeclaration2, formula));
    ...
    public static OperableFormula ForAll(OperableVariableDeclaration variableDeclaration1, OperableVariableDeclaration variableDeclaration2, OperableVariableDeclaration variableDeclaration3, OperableFormula formula) =>
        new OperableUniversalQuantification(variableDeclaration1, new OperableUniversalQuantification(variableDeclaration2, new OperableUniversalQuantification(variableDeclaration3, formula)));
    ...
    public static OperableFormula ThereExists(OperableVariableDeclaration variableDeclaration, OperableFormula formula) =>
        new OperableExistentialQuantification(variableDeclaration, formula);
    ...
    public static OperableFormula ThereExists(OperableVariableDeclaration variableDeclaration1, OperableVariableDeclaration variableDeclaration2, OperableFormula formula) =>
        new OperableExistentialQuantification(variableDeclaration1, new OperableUniversalQuantification(variableDeclaration2, formula));
    ...
    public static OperableFormula ThereExists(OperableVariableDeclaration variableDeclaration1, OperableVariableDeclaration variableDeclaration2, OperableVariableDeclaration variableDeclaration3, OperableFormula formula) =>
        new OperableExistentialQuantification(variableDeclaration1, new OperableExistentialQuantification(variableDeclaration2, new OperableExistentialQuantification(variableDeclaration3, formula)));
    ...
    /// <summary>
    /// Surrogate for <see cref="Formula"/> instances that defines |, &amp; and ! operators to create disjunctions, conjunctions and negations respectively.
    /// Instances are implicitly convertible to the equivalent <see cref="Formula"/> instance.
    /// </summary>
    public abstract class OperableFormula
    {
        /// <summary>
        /// Composes two <see cref="OperableFormula"/> instances together by forming a conjunction from them.
        /// </summary>
        ...
        public static OperableFormula operator &(OperableFormula left, OperableFormula right) => new OperableConjunction(left, right);

        /// <summary>
        /// Composes two <see cref="OperableFormula"/> instances together by forming a disjunction from them.
        /// </summary>
        ...
        public static OperableFormula operator |(OperableFormula left, OperableFormula right) => new OperableDisjunction(left, right);

        /// <summary>
        /// Creates the negation of an <see cref="OperableFormula"/> instance.
        /// </summary>
        /// <param name="operand">The formula to negate.</param>
        /// <returns>A new <see cref="OperableNegation"/> instance.</returns>
        public static OperableFormula operator !(OperableFormula operand) => new OperableNegation(operand);

        /// <summary>
        /// Implicitly converts an <see cref="OperableFormula"/> instance to an equivalent <see cref="Formula"/>.
        /// </summary>
        /// <param name="formula">The formula to convert.</param>
        public static implicit operator Formula(OperableFormula formula) => formula switch
        {
            ...
            OperableExistentialQuantification existentialQuantification => new ExistentialQuantification(existentialQuantification.Variable, existentialQuantification.Formula),
            OperableImplication implication => new Implication(implication.Antecedent, implication.Consequent),
            OperableNegation negation => new Negation(negation.Formula),
            OperablePredicate predicate => new Predicate(predicate.Identifier, predicate.Arguments.Select(a => (Term)a).ToArray()),
            OperableUniversalQuantification universalQuantification => new UniversalQuantification(universalQuantification.Variable, universalQuantification.Formula),
            _ => throw new ArgumentException($"Unsupported OperableFormula type '{formula.GetType()}'", nameof(formula))
        };
    }

    /// <summary>
    /// Surrogate for <see cref="Conjunction"/> instances that derives from <see cref="OperableFormula"/> and can thus be operated on with the |, &amp; and ! operators
    ...
    /// <see cref="OperableFormula"/> instances.
    /// </summary>
    public sealed class OperableConjunction : OperableFormula
    {
        internal OperableConjunction(OperableFormula left, OperableFormula right) => (Left, Right) = (left, right);

        internal OperableFormula Left { get; }

        internal OperableFormula Right { get; }
    }

    /// <summary>
    /// Surrogate for <see cref="Conjunction"/> instances that derives from <see cref="OperableFormula"/> and can thus be operated on with |, &amp; and ! operators
    ...
    /// <see cref="OperableFormula"/> instances.
    /// </summary>
    public sealed class OperableDisjunction : OperableFormula
    {
        internal OperableDisjunction(OperableFormula left, OperableFormula right) => (Left, Right) = (left, right);
    ...
    /// <summary>
    /// Surrogate for <see cref="Equivalence"/> instances that derives from <see cref="OperableFormula"/> and can thus be operated on with |, &amp; and ! operators
    ...
    /// <see cref="Iff"/> method.
    /// </summary>
    public sealed class OperableEquivalence : OperableFormula
    {
        internal OperableEquivalence(OperableFormula left, OperableFormula right) => (Left, Right) = (left, right);
    ...
    /// <summary>
    /// Surrogate for <see cref="ExistentialQuantification"/> instances that derives from <see cref="OperableFormula"/> and can thus be operated on with |, &amp; and ! operators
    /// to create disjunctions, conjunctions and negations respectively. N.B. constructor is intentionally not public - should only be created via the
    /// <see cref="ThereExists(OperableVariableDeclaration, OperableFormula)"/> method (or its overrides).
    /// </summary>
    public sealed class OperableExistentialQuantification : OperableFormula
    {
        internal OperableExistentialQuantification(OperableVariableDeclaration variable, OperableFormula formula) => (Variable, Formula) = (variable, formula);

        internal OperableVariableDeclaration Variable { get; }

        internal OperableFormula Formula { get; }
    }
    ...
    /// <summary>
    /// Surrogate for <see cref="Implication"/> instances that derives from <see cref="OperableFormula"/> and can thus be operated on with |, &amp; and ! operators
    /// to create disjunctions, conjunctions and negations respectively. N.B. constructor is intentionally not public - should only be created via the
    /// <see cref="If"/> method.
    /// </summary>
    public sealed class OperableImplication : OperableFormula
    {
        internal OperableImplication(OperableFormula antecedent, OperableFormula consequent) => (Antecedent, Consequent) = (antecedent, consequent);

        internal OperableFormula Antecedent { get; }

        internal OperableFormula Consequent { get; }
    }

    /// <summary>
    /// Surrogate for <see cref="Negation"/> instances that derives from <see cref="OperableFormula"/> and can thus be operated on with |, &amp; and ! operators
    /// to create disjunctions, conjunctions and negations respectively. N.B. constructor is intentionally not public - should only be created via the ! operator acting on
    /// <see cref="OperableFormula"/> instances.
    /// </summary>
    public sealed class OperableNegation : OperableFormula
    {
        internal OperableNegation(OperableFormula formula) => Formula = formula;

        internal OperableFormula Formula { get; }
    }

    /// <summary>
    /// Surrogate for <see cref="Predicate"/> instances that derives from <see cref="OperableFormula"/> and can thus be operated on with |, &amp; and ! operators
    ...
    /// </summary>
    public class OperablePredicate : OperableFormula
    {
        ...
        /// <summary>
        /// Implicitly converts an <see cref="OperablePredicate"/> instance to an equivalent <see cref="Formula"/>.
        /// </summary>
        /// <param name="predicate">The operable predicate to convert.</param>
        public static implicit operator Formula(OperablePredicate predicate) => new Predicate(predicate.Identifier, predicate.Arguments.Select(a => (Term)a).ToArray());
    }
...
#pragma warning disable CS0660, CS0661
    ...
    // hopefully a good example of why I want to keep this approach to creating formulas well away from the actual formula classes..
    public abstract class OperableTerm
    {
        ...
        /// <returns>A new <see cref="OperablePredicate"/> instance.</returns>
        public static OperableFormula operator ==(OperableTerm left, OperableTerm right) => new OperablePredicate(EqualityIdentifier.Instance, left, right);
        ...
        /// <returns>A new <see cref="OperableNegation"/> instance.</returns>
        public static OperableFormula operator !=(OperableTerm left, OperableTerm right) => new OperableNegation(new OperablePredicate(EqualityIdentifier.Instance, left, right));
...
#pragma warning restore CS0660, CS0661
    ...
    /// <summary>
    /// Surrogate for <see cref="UniversalQuantification"/> instances that derives from <see cref="OperableFormula"/> and can thus be operated on with |, &amp; and ! operators
    /// to create disjunctions, conjunctions and negations respectively. N.B. constructor is intentionally not public - should only be created via the
    /// <see cref="ForAll(OperableVariableDeclaration, OperableFormula)"/> method (or its overrides).
    /// </summary>
    public sealed class OperableUniversalQuantification : OperableFormula
    {
        internal OperableUniversalQuantification(OperableVariableDeclaration variable, OperableFormula formula) => (Variable, Formula) = (variable, formula);
