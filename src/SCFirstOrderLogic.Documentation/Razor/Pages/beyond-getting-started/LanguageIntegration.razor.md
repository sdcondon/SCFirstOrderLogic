...
The only perhaps non-obvious part of this one is the classes in the LanguageIntegration namespace, which allow for specifying FoL formulas as LINQ expressions.
That is, rather than directly giving the knowledge base formulas of first order logic as instances of the Formula class, we (represent domains as IEnumerable&lt;TElement&gt; and) `Tell` an `IKnowledgeBase` bool-valued expressions that are guaranteed to be true for all models that it will be `Ask`ed about - which the knowledge base then converts into the entailed formulas of first order logic.

Benefits of using LINQ expressions:

* Your formulas of propositional logic can be expressed in the familiar, plain-old C#, with the operators you would expect (e.g. `&&`, `||` and `!`).
...
* Also as mentioned above, when `Tell`ing facts to knowledge base instances, the fundamental rule is that FoL formulas are entailed by the assertion that the provided C# expression is guaranteed to evaluate to "true" for all models that the knowledge base will be `Ask`ed about.
...
The mapping between bool-valued C# expressions (that act on the TDomain) and FoL formulas can be found in the table below:

| **FoL** | **FoL Syntax** | **C# Expression** |
| --- | --- | --- |
|Conjunction|`{formula} ∧ {formula}`|`{expression} {&& or &} {expression}`|
|Disjunction|`{formula} ∨ {formula}`|`{expression} {\|\| or \|} {expression}`|
|Material equivalence|`{formula} ⇔ {formula}`|`Operators.Iff({expression}, {expression})` *|
|Material implication|`{formula} ⇒ {formula}`|`Operators.If({expression}, {expression})` *|
|Negation|`¬{formula}`|`!{expression}`|
|Existential quantification|`∃ {variable}, {formula}`|`{domain}.Any({variable} => {expression})` †|
|Universal quantification|`∀ {variable}, {formula}`|`{domain}.All({variable} => {expression})` †|
...
A couple of notes:
* Of course, existential quantification can be used in many cases `d => d.Any(x => x.Predicate1 && ..)`. We only need a constant when we need to refer to the same entity across different formulas.
